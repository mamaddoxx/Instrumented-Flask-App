...
class LoadTester:
    ...
    def test_cache_hit(self) -> None:
        # First, add a set of items to Redis for the test
        hit_keys = [f"cache_hit_key_{i}" for i in range(3)]
        for key in hit_keys:
            try:
                payload = {"key": key, "value": f"value_for_{key}"}
                requests.post(f"{self.base_url}/items", json=payload, timeout=5)
            except requests.exceptions.RequestException:
                pass
        while time.time() - self.start_time < self.duration:
            try:
                ...
                requests.get(f"{self.base_url}/items/cache_hit_key_{key_num}", timeout=5)
                time.sleep(0.1)
            except requests.exceptions.RequestException:
                pass
        print("Cache hit endpoint completed")
